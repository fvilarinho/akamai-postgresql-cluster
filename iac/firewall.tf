...
resource "linode_firewall" "clusterNodes" {
  ...
  label           = "${each.key}-cn-fw"
  tags            = concat(var.settings.general.tags, each.value.tags)
  inbound_policy  = "DROP"
...
resource "linode_firewall" "nodeBalancers" {
  ...
  inbound {
    ...
    ipv4     = concat(each.value.allowedIps.ipv4,
                      [
                        ...
                        "${linode_instance.grafana.ip_address}/32",
                        "${linode_instance.grafana.private_ip_address}/32",
  ...
  nodebalancers = [
    ...
    data.external.fetchNodeBalancers[each.key].result.monitoringId
  ]
...
resource "linode_firewall" "grafana" {
  ...
  inbound {
    ...
    protocol = "TCP"
    ports    = "22,80,443"
    ipv4     = concat(var.settings.grafana.allowedIps.ipv4,
                      [ "${jsondecode(data.http.myIp.response_body).ip}/32" ])
    ipv6     = concat(var.settings.grafana.allowedIps.ipv6,
                      [ "::1/128" ])
  }
